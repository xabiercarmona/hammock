...
public class AttachmentStreamFactoryTest extends BasicDatastoreTestBase {
    ...
    public void testPreparedAttachmentWritesUnencryptedEncodedStream()
        ...
        System.err.println("testPreparedAttachmentWritesUnencryptedEncodedStream: " + out.getClass().getPackage());
        IOUtils.copy(new FileInputStream(plainText), out);
        out.close();

        System.err.println("testPreparedAttachmentWritesUnencryptedEncodedStream: "+actualOutput.getAbsolutePath());
        Assert.assertTrue("File does not exist " + actualOutput.getAbsolutePath(), actualOutput.exists());

        Assert.assertTrue("Writing to unencrypted, encoded stream didn't give correct output " + actualOutput.getAbsolutePath(),
                IOUtils.contentEquals(new FileInputStream(actualOutput), new FileInputStream(zippedPlainText)));
        IOUtils.contentEquals(new GZIPInputStream(new FileInputStream(actualOutput)), new GZIPInputStream(new FileInputStream(zippedPlainText)));
